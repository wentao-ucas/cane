# -*- coding: utf-8 -*-
"""ENSO 滞后效应分析: 6M 滑动 ONI -> 后续 6-12 月玉米价前瞻收益
注意: 玉米 La Niña 利多(美玉米带干旱减产)，预期回归符号与糖相反
"""
import sys, io
...
m = pd.read_csv('data/corn_oni_merged.csv', parse_dates=['date']).set_index('date')

m['oni_6m'] = m['oni'].rolling(6).mean()
m['fwd_6m']  = m['c_close'].shift(-6)  / m['c_close'] - 1
m['fwd_12m'] = m['c_close'].shift(-12) / m['c_close'] - 1
m['fwd_18m'] = m['c_close'].shift(-18) / m['c_close'] - 1
...
print("=== 按 6 个月平滑 ONI 分组：未来 6/12/18 月大商所玉米前瞻收益 ===")
...
crosses = m[m['oni_cross']][['oni','oni_6m','c_close']]
for d, r in crosses.iterrows():
    fwd_6 = m.loc[d, 'fwd_6m']
    fwd_12 = m.loc[d, 'fwd_12m']
    ...
    print(f"  {d.strftime('%Y-%m')} {state} 突破 (ONI={r['oni']:+.2f}, 6M均={r['oni_6m']:+.2f}, 玉米{r['c_close']:.0f}) -> 6M:{fwd6}, 12M:{fwd12}")

m[['c_close','c_ret%','oni','oni_6m','bucket','fwd_6m','fwd_12m','fwd_18m']].round(2).to_csv('data/corn_oni_lag_analysis.csv', encoding='utf-8-sig')
print("\n保存: data/corn_oni_lag_analysis.csv")
